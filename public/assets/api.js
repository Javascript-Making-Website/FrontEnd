...
// ✅ 내 재생목록 목록 조회 (likes / liked / items 포함)
async function playlists() {
  try {
    return await fx(`${BASE}/api/playlists`); // 서버 API 호출
  } catch {
    // 서버 실패 시 로컬 폴백 재생목록 반환
  ...
  try {
    return await fx(`${BASE}/api/playlists`, {
      ...
      body: JSON.stringify({
        title,
        isPublic,
        themeColor: themeColor || '#22d3ee' // 색상을 안 넘겼으면 기본색
      })
    });
  } catch {
    // 서버 실패 시 UI가 터지지 않도록 폴백
...
async function deletePlaylist(id) {
  try {
    return await fx(`${BASE}/api/playlists/${id}`, { method: 'DELETE' });
  } catch {
    // 폴백: 로컬 재생목록 비우기
    Playlist.set([]);
    return { ok: true, fallback: true };
...
async function removeFromPlaylist({ playlistId, trackId }) {
  try {
    ...
    // 서버 실패 시 로컬 재생목록에서 제거
    Playlist.remove(trackId);
    return { ok: true, fallback: true };
  }
}

// ⭐ 특정 재생목록 상세 조회 (player에서 사용 예정)
async function playlistDetail(id) {
  try {
    return await fx(`${BASE}/api/playlists/${id}`); // 서버 API 호출
  } catch {
    // 서버 실패 시 로컬 가짜 재생목록 반환
    return {
      id: 0,
...
async function togglePlaylistLike(id) {
  try {
    return await fx(`${BASE}/api/playlists/${id}/like`, { method: 'POST' });
  } catch {
    // 서버 실패 시 로컬 폴백 (좋아요 상태 저장 안 함)
    return { ok: true, liked: true, likes: 0, fallback: true };
  }
}

// ✅ 공개 재생목록 목록 조회 (최근 / 인기)
//    사용 예: API.publicPlaylists({ sort: 'recent' | 'popular', limit })
async function publicPlaylists({ sort = 'recent', limit = 20 } = {}) {
  try {
    const u = new URL(`${BASE}/api/playlists/public`);
    if (sort) u.searchParams.set('sort', sort); // 정렬 방식
    if (limit) u.searchParams.set('limit', String(limit)); // 조회 제한
    return await fx(u);
  } catch {
    // 서버 실패 시 빈 배열 반환
    return { items: [] };
